...
class ADC(Dataset):
    def __init__(self, mode, dataset_path="../data"):
        ...
        self.train_path = self.root + '/ADC/train/image'
        self.train_seglabel_path = self.root + '/ADC/train/label'
        self.train_classlabel_path = self.root + '/train_classify_label_ADC.csv'
        self.test_path = self.root + '/ADC/val/image'
        self.test_seglabel_path = self.root + '/ADC/val/label'
    ...
    def __getitem__(self, item):
        ...
        # if self.mode == 'val':
        #     class_lab = self.classlabel_list[item+334] #  0.8*增强后的数据集 = int（0.8*（209*2）aug+ori
        # else:
