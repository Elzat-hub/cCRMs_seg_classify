...
# Single tumor DWI MRI:200 ~ 420 Set:150 ~ 650 Size:32x256x256
train_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_train/image/DWI"
train_label_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_train/label/DWI"
val_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_val/image/DWI"
val_label_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_val/label/DWI"
...
if __name__ == '__main__':
    ...
    # # training dataset augmentation Single tumor DWI
    ...
    #     os.makedirs('../data/DWI/train/image', exist_ok=True)
    #     os.makedirs('../data/DWI/train/label', exist_ok=True)
    #     nib.save(new_img, '../data/DWI/train/image/aug_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    #     nib.save(new_lab, '../data/DWI/train/label/aug_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    # print("Training dataset augmentation transformed FINISH.")

    # # training dataset original crop Single tumor DWI
    # for i in range(len(list_train)):
    ...
    #     os.makedirs('../data/DWI/train/image', exist_ok=True)
    #     os.makedirs('../data/DWI/train/label', exist_ok=True)
    #     nib.save(new_img, '../data/DWI/train/image/ori_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    #     nib.save(new_lab, '../data/DWI/train/label/ori_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    # print("Training dataset original cropped FINISH.")

    # valing dataset ori Single tumor DWI     这是有用的，  可以跟上面的代码一起用，val的数据进行裁剪成统一  256*256的，   不同序列的大小看Vnet
    for i in range(len(list_val)):
        ...
        new_img = nib.Nifti1Image(img_save, np.eye(4))
        new_lab = nib.Nifti1Image(lab_save, np.eye(4))
        os.makedirs('../data/DWI/val/image', exist_ok=True)
        os.makedirs('../data/DWI/val/label', exist_ok=True)
        nib.save(new_img,
         '../data/DWI/val/image/t_{}.nii.gz'.format(list_val[i].split("/")[-1].split(".")[0]))
        nib.save(new_lab,
         '../data/DWI/val/label/t_{}.nii.gz'.format(list_val[i].split("/")[-1].split(".")[0]))
