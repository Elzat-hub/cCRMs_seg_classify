...
train_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_train/image/T1WI"
train_label_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_train/label/T1WI"
val_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_val/image/T1WI"
val_label_path = "/home7/yilizhati/data/301kidney_tumor/singletumor_val/label/T1WI"
...
if __name__ == '__main__':
    ...
    #     os.makedirs('../data/T1WI/train/image', exist_ok=True)
    #     os.makedirs('../data/T1WI/train/label', exist_ok=True)
    #     nib.save(new_img, '../data/T1WI/train/image/aug_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    #     nib.save(new_lab, '../data/T1WI/train/label/aug_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    ...
    #     os.makedirs('../data/T1WI/train/image', exist_ok=True)
    #     os.makedirs('../data/T1WI/train/label', exist_ok=True)
    #     nib.save(new_img, '../data/T1WI/train/image/ori_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    #     nib.save(new_lab, '../data/T1WI/train/label/ori_{}.nii.gz'.format(list_train[i].split("/")[-1].split(".")[0]))
    # print("Training dataset original cropped FINISH.")

    # valing dataset ori Single tumor T1WI     这是有用的，  可以跟上面的代码一起用，val的数据进行裁剪成统一  256*256的，   不同序列的大小看Vnet
    for i in range(len(list_val)):
        ...
        new_img = nib.Nifti1Image(img_save, np.eye(4))
        new_lab = nib.Nifti1Image(lab_save, np.eye(4))
        os.makedirs('../data/T1WI/val/image', exist_ok=True)
        os.makedirs('../data/T1WI/val/label', exist_ok=True)
        nib.save(new_img,
         '../data/T1WI/val/image/t_{}.nii.gz'.format(list_val[i].split("/")[-1].split(".")[0]))
        nib.save(new_lab,
         '../data/T1WI/val/label/t_{}.nii.gz'.format(list_val[i].split("/")[-1].split(".")[0]))
