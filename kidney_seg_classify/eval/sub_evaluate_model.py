...
from sub_experiment.sub_data_creater import create_data
...
def visualize_segmentation_results(model, data_loader, config, sequence_name):
    ...
    # output_dir = f'./sub_seg_results_train_{sequence_name}'    # 训练集与验证集的保存路径
    output_dir = f'./sub_seg_results_test_{sequence_name}'    # 测试集保存路径
    ...
    # 保存空白病例列表
    with open(f"sub_{sequence_name}_empty_cases_test.txt", "w") as f:
...
def main():
    ...
    # train_generator, _ = create_data(args, config)  
    # _, val_generator, _ = create_data(args, config)  
    _, test_generator = create_data(args, config)  
    ...
    save_results(results, f"sub_evaluation_results_{sequence_name}.txt")
