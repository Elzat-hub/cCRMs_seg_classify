...
test_file_path = "../data/features/test_features.csv"
...
# 根据 ROC 曲线找到最佳阈值
fpr, tpr, thresholds = roc_curve(y_test, rf_prob_final)
...
# 保存预测结果到 CSV 文件
predictions_path = os.path.join('final_performance', 'test_predictions.csv')
...
performance_summary_path = os.path.join('final_performance', 'model_summary.txt')
with open(performance_summary_path, 'w') as f:
    f.write("Random Forest Model Summary\n")
    f.write("===========================\n\n")
    
    f.write("1. Model Performance:\n")
    f.write(f"   Accuracy: {rf_accuracy_final:.4f} (95% CI: [{ci_lower:.4f}, {ci_upper:.4f}])\n")
    ...
    f.write("3. Feature Importance (Top 13):\n")
    for _, row in feature_importance_final.head(13).iterrows():
...
roc_values_path = os.path.join('final_performance', 'roc_values.csv')
...
# 保存ROC曲线图
roc_path = os.path.join('final_performance', 'roc_curve.png')
...
# 保存混淆矩阵图
cm_path = os.path.join('final_performance', 'confusion_matrix.png')
...
plt.barh(range(len(top_13_features)), top_13_features['importance'], align='center', height=0.6, color='#FF6347')
...
plt.box(on=True)
# 保存特征重要性图
feature_importance_path = os.path.join('final_performance', 'feature_importance.png')
...
# 保存SHAP摘要图
shap_summary_path = os.path.join('final_performance', 'shap_summary_plot.png')
...
with open(performance_summary_path, 'a') as f:
    ...
    for _, row in feature_importance_shap.head(13).iterrows():
        f.write(f"   {row['feature']}: {row['importance']:.4f}\n")
